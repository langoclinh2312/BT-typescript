import { Accountss } from './method/Accountss.js';
import { Accounts } from './declaration/Accounts.js';
declare var $: any;
var student = new Accountss();
student.getAll();
//thêm mới
$(document).on('click', '.btn-add', function (ev: any) {
    ev.preventDefault();
    ...
    if (student.validate(acc)) {
        student.create(acc);
        student.getAll();
        (document.getElementById('creatform') as HTMLFormElement).reset();
    }
});
